...
//绑定Dom事件
var button1 = document.getElementById('setLoc');
var listener1 = AMap.event.addDomListener(button1, 'click', bind);
var button2 = document.getElementById('clearLoc');
...
function regeocoder(e) {  //逆地理编码
    ...
    geocoder.getAddress(e.lnglat, function (status, result) {
        ...
        if (status === 'complete' && result.info === 'OK') {
            ...
            setAddress(address, e.lnglat.getLng(), e.lnglat.getLat());
            map.setFitView();
            $("#type").val("1");
        } else {
            getLoc(e);
...
function getLoc(e) {
    $.ajax({
        url: "/post/getLocAddress",
        async: true,
        data: {"lng": e.lnglat.getLng(), "lat": e.lnglat.getLat()},
        type: "POST",
        dataType: "json",
...
function setAddress(address, lng, lat) {
    $("#myLoc").val(address);
    $("#loc_adr").text(address);
    $("#loc_lng").val(lng);
    $("#loc_lat").val(lat);
    locMarker = new AMap.Marker({//加点
        position: [lng, lat]
    });
    map.add([locMarker]);
}
...
$(".descripe-box").slide({
    mainCell: ".descripe-box-main",
    titCell: '.descripe-box-type ul li',
    trigger: 'click'
});
$(".popup-upload .upload-list ul li").click(function () {
    $(this).toggleClass("on");
});
$(".descripe-box-footer .tags .lr").click(function () {
    $(".descripe-box-footer .tags .text-box").toggleClass("hide");
});
$(".adr").click(function () {
